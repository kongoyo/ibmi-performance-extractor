...
| 2026-08-08 | `QAPMJOBL.JBIPAF`/`JBIPAD`/`JBIPPT`（RCA 新增 Remote Address 欄位，`jobsQuery` 首次引用） | 第一版實作用 `MAX(HEX(JBIPAF))`、`MAX(HEX(JBIPAD))` 各自獨立取值（誤以為同一 Job 同一 Interval 只有一列）。連線 clark75 `QPFRDATA.QAPMJOBL` 針對 `QZDASOINIT/QUSER/846143` 08/07 17:00 實測，第一版解出 family=`18`（IPv6）但位址全為 0（`0000:...:0000`），明顯異常；查 `JBTHDF`（secondary thread flag）欄位說明後確認同一 Job 同一 Interval 可能有多筆列（次執行緒），`MAX()` 對每欄各自取值會把不同列的 family 與位址位元組錯誤配對。 | 改用 `ROW_NUMBER() OVER (PARTITION BY INTNUM,JBNAME,JBUSER,JBNBR ORDER BY JBTHDF ASC, ...)` 從同一列一次取出 family+位址+port（優先取 primary thread, `JBTHDF=0`），修正後同一 Job 同一時段解出穩定值 `10.255.0.197:52225`（IPv4-mapped IPv6 `::ffff:0aff:00c5`，`jobRanker.js` 的 `decodeRemoteAddress()` 已還原成一般 IPv4 顯示）。`npm run validate` 163 項全數通過（此欄位不在既有 Test Area 斷言範圍內，僅確認新 SQL 不破壞既有查詢）。 |
| 2026-08-08 | `QSYS2.HISTORY_LOG_INFO`（QHST 系統歷史紀錄 SQL Service，`historyLogFetcher.js` 首次引用，取代手動 `DSPLOG` 匯出） | 使用者提供 clark75 08/07 全天的 `DSPLOG OUTPUT(*PRINT)` 文字檔手動交叉比對出 `QZDASOINIT/QUSER/846143` 的真實使用者（`CLARK`）與登入/登出紀錄後，要求改用即時查詢自動化、且只抓異常 Job 的時間區間而非整天。連線 clark75 `QPFRDATA` 實測 `SELECT * FROM TABLE(QSYS2.HISTORY_LOG_INFO(START_TIME=>...,END_TIME=>...))`：欄位 `FROM_JOB_NAME`/`FROM_JOB_USER`/`FROM_JOB_NUMBER`/`FROM_USER` 皆為結構化欄位（不需像文字檔那樣正則解析job身分），`WHERE FROM_JOB_NUMBER='846143'` 精準篩出與手動 grep 完全一致的 2 筆紀錄（connect 16:40:50、end 19:25:15，210.744 秒、結束碼 10）；但 `MESSAGE_TEXT` 沒有結構化的遠端 IP／CPU秒數／結束碼欄位，且本次連線 session 回傳的是**已翻譯成繁體中文**的訊息文字（非官方文件慣用的英文），因此這些欄位仍需對 `MESSAGE_TEXT` 做正則擷取，且需同時支援中英文兩種措辭。 | `scripts/queries.js` 新增 `historyLogQuery()`；`scripts/historyLogFetcher.js` 的正則擷取（IP 用語言無關的 `\d+\.\d+\.\d+\.\d+` 樣式、秒數/結束碼/子系統各自嘗試英文與中文樣式）。查詢視窗預設縮限在該 Job 已知活躍時段附近（±60 分鐘起），若未涵蓋到 `jobEnd` 事件會以 `stepMinutes` 一小時一小時往外擴張，而非直接查整天，兩者皆已用真機資料驗證（連線 clark75 執行 `npm run rca`，含觸發擴張情境）。 |
...
| `JBTHDF` | Secondary thread flag（0=primary/task thread, 1=secondary thread）| ⚠️ 同一 Job 同一 Interval 可能有多筆列（見下方 2026-08-08 變更記錄），需要「單一連線身分」欄位（如 `JBIPAF`/`JBIPAD`）時必須用這個欄位挑出 primary thread 那一列，不能對每欄各自 `MAX()` |
| `JBIPAF` | Remote IP address family flag（X'00'=未設定, X'02'=IPv4, X'18'=IPv6）| 原始二進位，SQL 端須用 `HEX()` 轉字串才能安全取出，見 `queries.js` 的 `jobsQuery` |
| `JBIPAD` | Remote IP address 二進位形式（IPv4 4 bytes / IPv6 16 bytes）| 同上，`HEX()` 轉字串後由 `jobRanker.js` 的 `decodeRemoteAddress()` 解碼成人類可讀位址；IPv4-mapped IPv6（`::ffff:a.b.c.d`）會還原成一般 IPv4 顯示 |
| `JBIPPT` | Remote port number | — |
